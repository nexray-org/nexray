...
export function Spinner({ size = 'sm', ...props }: ISpinner) {
    function getSize() {
        switch (size) {
            case 'sm':
                return 16;
    ...
    return (
        <svg height={`${getSize()}px`} width={`${getSize()}px`} viewBox='0 0 16 16' fill='none' {...props} className={clsx('animate-spin', props.className)}>
