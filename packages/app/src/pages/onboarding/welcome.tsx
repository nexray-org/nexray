...
export default function Welcome() {
    ...
    const steps = [
        {
            ...
            children: (
                <>
                    <div className='mb-3'>
                        <span>
                            In your Next.js 13 project, install the Nexray library with your preferred package manager. The provided snippets use{' '}
                            <Code>npm</Code>.
                        </span>
                    </div>
        ...
        },
        {
            ...
            children: (
                <>
                    <div className='mb-3'>
                        <span>
                            For each <em>page</em> file in your <Code>app/</Code> directory, wrap the default export with <Code>nexray</Code>.
                        </span>
                    </div>
                    <WrapPages />
                </>
    ...
    return (
        <Root>
            <Scrollbars renderThumbVertical={thumbRenderer} renderThumbHorizontal={thumbRenderer} universal>
                <div className='flex-center py-[30px]'>
                    <div className='w-full max-w-[500px]'>
                        ...
                        <div className='flex space-x-6'>
                            <DataSourceButton selected={selectedDataSource === 'local'} title='Local Server' onSelect={() => setSelectedDataSource('local')}>
                            ...
                            <Disclosure defaultOpen={!!ele.defaultOpen} key={i}>
                                {({ open }) => (
                                    <>
                                        ...
                                        >
                                            <span>{ele.panel}</span>
                                            <BsChevronUp className={clsx('transform h-4 w-4 my-auto', !open && 'rotate-180')} />
                                        </Disclosure.Button>
                                        <Disclosure.Panel className='px-1 pt-4 text-sm text-g-primary-200 tracking-tight'>{ele.children}</Disclosure.Panel>
                                    </>
                                )}
                            </Disclosure>
                        ))}
                        <div className='mt-4'>
                            <span className='text-g-primary-300 font-semibold tracking-tight text-sm'>
                                <span className='text-g-primary-50'>You're all set.</span> Start the development server on this machine and navigate to a page.
                                Traces will automatically appear here.
                            </span>
                        </div>
...
function DataSourceButton({ selected, title, children, onSelect, className, disabled }: IDataSourceButton) {
    ...
    return (
        <div className='flex basis-1/2'>
            <button className={clsx(...dataSourceClassName, className, disabled && 'pointer-events-none')} onClick={onSelect}>
                {disabled && (
                    <>
                ...
                <div className={clsx('w-full flex items-center justify-between mb-4', disabled && 'brightness-50')}>
                    <span className={clsx('text-inherit tracking-tight font-bold')}>{title}</span>
                ...
                <div className={clsx('w-full h-full block text-sm', disabled && 'brightness-50')}>{children}</div>
            </button>
        </div>
    );
}
