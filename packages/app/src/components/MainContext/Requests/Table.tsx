...
export default function Table() {
    ...
    return (
        <div className='relative'>
            <Scrollbars style={{ height: height - 46 - 10 }} renderThumbVertical={thumbRenderer} renderThumbHorizontal={thumbRenderer} width={200} universal>
                <table className='w-full table-fixed'>
                    <thead className='sticky top-0 bg-g-primary-900 after:absolute after:left-0 after:w-full after:border-b after:border-b-g-primary-700'>
                        <tr>
                            <th scope='col' colSpan={6} className={clsx(headerClassName, 'text-left')}>
                                URL
                            </th>
                            <th scope='col' colSpan={2} className={clsx(headerClassName, 'text-center')}>
                                OK
                            </th>
                            <th scope='col' colSpan={2} className={clsx(headerClassName, 'text-center')}>
                                Type
                            </th>
                            <th scope='col' colSpan={3} className={clsx(headerClassName, 'text-center')}>
                                Size
                            </th>
                    ...
                    <tbody>
                        {Object.values(activeItem!.fetches).map((ele) => (
                            <Fragment key={ele.id}>
                                <tr
                                    className={clsx('group border-b border-b-g-primary-700')}
                                    onClick={() => {
                                        setSelectedRequestsTab('insights');
                                        setSelectedRequestForInsightId(ele.id);
                                    }}
                                >
                                    ...
                                    <td colSpan={2} className={clsx(rowClassName, 'text-center')}>
                                        <div className='flex-center'>
                                            <span className='uppercase text-[10px] tracking-wide leading-[10px] font-bold flex-center py-1 px-1.5 bg-g-success-300 text-white rounded-sm'>
                                                {ele.requestInit?.method || 'GET'}
                                            </span>
                                        </div>
                                    </td>
                                    <td colSpan={3} className={clsx(rowClassName, 'text-center')}>
                                        {ele.response?.size ? formatBytes(ele.response.size) : ''}
                                    </td>
                                    <td onClick={() => {}} className={clsx(rowClassName, 'pl-2 pr-0 text-right')} colSpan={1}>
