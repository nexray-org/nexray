        ...
        <Modal visible={isInsightFilterDialogOpen} onClose={() => setIsInsightFilterDialogOpen(false)} width='680px'>
            ...
            <Modal.Content pt={0.4}>
                <p className='text-sm inline-block'>
                    Filter JSON objects with{' '}
                    <Link target={'_blank'} color href='https://goessner.net/articles/JsonPath/index.html#e2'>
                        JSONPath
                    </Link>
                    , a powerful traversal and parsing syntax. These examples demonstrate various functions of JSON structure that represents a book store:
                    <ul className='leading-none list-none'>
                        {[
                            ...
                            <li className='flex' key={ele.description}>
                                <span className='min-w-[275px] font-bold'>{ele.description}:</span>
                                <code>{ele.code}</code>
                            </li>
                        ))}
                    </ul>
                    See more{' '}
                    <Link target={'_blank'} color href='https://support.smartbear.com/alertsite/docs/monitors/api/endpoint/jsonpath.html'>
                        examples here
                    </Link>
                    .
                </p>
