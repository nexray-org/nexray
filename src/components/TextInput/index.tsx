...
export function TextInput<T extends ElementType = 'input'>({
    ...
    const coreStyle = 'py-0.5 px-1 flex items-center border border-true-gray-800 group-[.is-error]:border-red-300 group-[.is-success]:border-green-400';
    const visualBaseStyle = 'px-1 text-sm text-gray-500';
    ...
    return (
        ...
        >
            {leadingVisual && (
                <span {...leadingVisualProps} className={clsx(visualBaseStyle, leadingVisualProps?.className)}>
                    {leadingVisual}
                </span>
            )}
            ...
            <span
                className={clsx(
                    'flex focus-within:z-10 w-full',
                )}
            >
                {leadingAddon && <span className='mr-2'>{leadingAddon}</span>}
                <Component type='text' {...props} className={clsx(props.className, 'appearance-none focus:outline-none bg-transparent text-sm w-full')} />
            ...
            {trailingVisual && (
                <span {...trailingVisualProps} className={clsx(visualBaseStyle, trailingVisualProps?.className)}>
                    {trailingVisual}
                </span>
            )}
        </span>
